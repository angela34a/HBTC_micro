...
For the majority of samples, the most prevailing class was Nanoarchaeia, from the
Archaea domain (37.9% ± 15.4% across all categories). 
The most abundant class from the Bacteria domain was Omnitrophia (8.10% ± 4.55%), 
and the fourth most abundant class overall also belonged to the Archaea domain - 
Nitrososphaeria (5.08% ± 4.67%). 
From the phylum Proteobacteria, the most abundant class was Gammaproteobacteria (7.96% ± 6.22%),
followed by Alphaproteobacteria (2.73% ± 3.14%).
...
Although this line seemed promising, the dataset did not have many samples which were outside of the lines of eliminating cross-section (unknown CH4 levels, unclassified order,  and/or 0 abundance in the sample). Taking that into account we can look closer into "Methylococcales", "Methylomirabilales" and "Methanosarciniales". However, even these three do not provide a robust correlation.
...
#change this code to contain:
#group_by(Order) %>% 
#cor_test(Temp, abund) %>% 
#ungroup() %>% 
#choose only significant

#the same for groups but anova
...
# ....
dplyr::select("asv", "Order")  -> tax_order
