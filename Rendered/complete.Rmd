...
tax_table <- read.csv("C:/Users/Angela Cukusic/Desktop/DS_analysis/data/tax_no_cont.csv")
...
Testing if the distribution of alpha diversity per temperature categories is significant.

```{r alpha-test}

# since there is only 1 sample in the 20< i must remove it before testing
data_for_alpha <- cbind( as.data.frame(ens), metadata$category) %>% 
  filter(metadata$category != "20<") %>% 
  rename("category" = "metadata$category")
...
Or, visually - with a new package:
```{r ggplot-stat}

alph <- cbind(as.data.frame(ens), as.factor(metadata$category)) %>% 
                as.data.frame() %>%
                rename("div"= "ens",
"temp"="as.factor(metadata$category)") 
...
Plot the diversity index on an interactive map.
...
We chose allt the aquifers that had more than 5 sampling points.
...
data_for_reg <- cbind(metadata) %>% dplyr::select(where(is.numeric)) %>% 
